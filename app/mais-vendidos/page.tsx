...
export default function BestSellersPage() {
  ...
  const sortedProducts = useMemo(() => {
    return [...storeProducts].sort((a, b) => b.sold - a.sold);
  }, [storeProducts]);
  ...
  }, [sortedProducts, search, category, selectedBrand]);
    ...
    <main className="min-h-screen bg-[#313338] text-[#f2f3f5]">
      ...
      <div className="grid min-h-screen md:grid-cols-[78px_250px_1fr]">
        ...
        <aside className="hidden bg-[#2b2d31] p-4 md:block">
          ...
          <Link href="/mais-vendidos" className="mb-3 block rounded-lg bg-[#23a559] px-3 py-2 text-center font-bold text-white transition">
            ⭐ Mais Vendidos
          </Link>
        ...
        <section className="bg-[#313338]">
          <header className="sticky top-0 z-20 border-b border-[#1e1f22] bg-[#313338] px-5 py-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h1 className="text-2xl font-black text-white">⭐ Mais Vendidos</h1>
                <p className="text-sm text-[#b5bac1]">Os campeões de vendas do nosso catálogo para você montar seu setup.</p>
              </div>
          ...
          <div className="p-5">
            ...
            {filtered.length === 0 ? (
              <div className="rounded-2xl bg-[#2b2d31] p-12 text-center">
                <p className="text-lg font-bold text-[#b5bac1]">Nenhum produto encontrado para os critérios selecionados.</p>
              </div>
            ) : (
                  ...
                  <div key={product.id} className="relative overflow-hidden rounded-2xl bg-[#2b2d31] shadow-lg flex flex-col justify-between">
                    ...
                    <div>
                      <div className="relative h-48 w-full bg-white p-3">
                        ...
                        />
                        <div className="absolute left-3 bottom-3 rounded-md bg-[#23a559] px-2.5 py-1 text-xs font-black text-white shadow-md">
                          #{idx + 1} Top Vendas
                        </div>
                      </div>
                      <div className="p-4">
                        ...
                        <p className="text-xs text-[#b5bac1] mt-1 font-bold">{product.sold} unidades vendidas</p>
                      </div>
                    </div>
